...
import { NGA } from '../nga';
import { syncCollectPosts } from '../commands/syncCollect';
import { Node } from '../models/node';

export default class CollectProvider extends BaseProvider {
  private rootElements: TreeNode[] = [];
  private _tipNode = new TreeNode('还没有收藏帖子', false);
  ...
  refreshNodeList() {
    ...
    console.log('refreshNodeList: ', customNodes);
  ...
  private async getElementData(root: TreeNode): Promise<TreeNode[]> {
    try {
      const topics = await syncCollectPosts();
